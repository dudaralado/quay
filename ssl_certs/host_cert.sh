...
openssl genrsa -out $Quay_Certs/$host_cert.key 2048
# Step 2.2 - Generate the Quay Operator Certificate Signing Request
openssl req -new -key  $Quay_Certs/$host_cert.key -out $Quay_Certs/$host_cert.csr -subj "/C=$COUNTRY/ST=$STATE/L=$location/O=Quay/OU=Docs/CN=$CN_FQDN"
#echo "Creating Quay Operator Certificate"
# Step 2.3 - Generate the Quay Operator Certificate
openssl x509 -req -in $Quay_Certs/$host_cert.csr -CA $Quay_Certs/rootCA.cert -CAkey $Quay_Certs/rootCA.key -CAcreateserial -out $Quay_Certs/$host_cert.cert -days $valid_days  -extensions v3_req -extfile $Quay_Certs/openssl.cnf
